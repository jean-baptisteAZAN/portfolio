<script lang="ts">
	...
	function moveShadow(event: MouseEvent) {
		...
		const maxDistance = Math.sqrt(Math.pow(faceRect.width, 2) + Math.pow(faceRect.height, 2)) * 0.05;
...
>
	<img src="/face.png" alt="Jean-Baptiste Azan, Développeur web Lille" class="w-60 h-60 rounded-full" />
