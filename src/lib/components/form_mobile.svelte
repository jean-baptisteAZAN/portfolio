<script>
	...
	const tab = [
		...
		},
		{
			...
			value: $new_value,
			class:
				'shadow appearance-none border rounded w-[45%] mr-[10%] py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mb-3',
			required: false,
			checked: false,
			id: 0
		},
		{
			...
			value: $new_value,
			class:
				'shadow appearance-none border rounded py-2 w-[45%] px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline',
			required: false,
			checked: false,
			id: 0
		},
	...
	function find_checkbox(id) {
		for (let i = 0; tab[i]; i++) if (tab[i].type == 'checkbox' && tab[i].id == id) return i;
		return -1;
	}
	...
	function change_checked(element) {
		...
		for (let i = 0; tab[i]; i++) {
			if (tab[i].type != 'checkbox' && tab[i].id > 0) {
				...
				if (
					pos_checkbox != -1 &&
					tab[pos_checkbox].type == 'checkbox' &&
					tab[pos_checkbox].checked == true
				)
					tab[i].required = true;
				else if (pos_checkbox != -1 && tab[pos_checkbox].checked == false) tab[i].required = false;
			}
		}
...
{#if visible == true}
	<div
		...
		class="md:hidden h-screen w-full fixed z-20 flex flex-col"
		in:fly={{ y: 1000, duration: 800 }}
		out:fly={{ y: 800, duration: 800 }}
	>
		<img
			src={croix}
			alt="close"
			class="h-[26px] w-[29px] mt-[30px] right-[12px] object-left absolute"
			on:click={change_value}
		/>
	...
	<div
		style="background-color:{couleur};"
		class="hidden md:flex h-screen w-full fixed z-20 flex-col"
...
{:else}
	<div
		...
		on:click={change_value}
		class="z-10 h-[92px] bottom-0 left-0 right-0 fixed md:hidden flex flex-col items-center justify-center"
	>
	...
	<div
		...
		on:click={change_value}
		class="z-10 w-[20%] h-full top-0 right-0 fixed hidden md:flex flex-row align-middleb content-center items-center justify-center"
	>
